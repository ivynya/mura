<script lang="ts">
	...
	import LinearMultiselect from './LinearMultiselect.svelte';
	...
	let inputTimes: number[] = $state([]);
	let inputTimeFrom: number = $derived(inputTimes[0] || 0);
	let inputTimeTo: number = $derived(inputTimes[inputTimes.length - 1] || 0);
...
<form onsubmit={submit}>
	...
	<label for="time_picker">
		Hour Options (Time From - Time To)
		<LinearMultiselect bind:times={inputTimes} />
	</label>
	<label for="tz_offset">
